...
# The conversion is needed
    ...
    # Finding matches
    p1 = subprocess.Popen("python2 associate.py " + dataset + "/rgb.txt " + dataset + "/depth.txt",
                          stdout=subprocess.PIPE, shell=True)
    p2 = subprocess.Popen("cut -d' ' -f 1,3 > " + dataset + "/matched", stdin=p1.stdout, stdout=subprocess.PIPE,
                          shell=True)
    p2.communicate()
    ...
    for z in x.split('\n'):
        ...
        if len(nums) > 1:
            ...
            # Copying images
            call("cp " + dataset + "/rgb/" + str(rgbName.rstrip()) + ".png " + dataset + "/rgb_%0.5d.png" % (i),
                 shell=True)
            call("cp " + dataset + "/depth/" + str(dName.rstrip()) + ".png " + dataset + "/depth_%0.5d.png" % (i),
                 shell=True)
            ...
            if i == 0:
                ...
                p2 = subprocess.Popen("cut -d' ' -f 2-8 > " + dataset + "/initialPosition", stdin=p1.stdout,
                                      stdout=subprocess.PIPE, shell=True)
                call("rm test init", shell=True)
            i = i + 1

    call(
        "rm -rf " + dataset + "/rgb " + dataset + "/depth " + dataset + "/rgb.txt " + dataset + "/depth.txt " + dataset + "/accelerometer.txt",
        shell=True)
